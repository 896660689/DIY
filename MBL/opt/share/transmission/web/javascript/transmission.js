...
Transmission.prototype =
{
	...
	****/
	...
	promptToRemoveTorrents:function(torrents)
	{
		if (torrents.length === 1)
		{
			var torrent = torrents[0],
			    header = '您确定要移除 ' + torrent.getName() + ' 吗?',
			    message = '移除后, 若要继续下载任务需提供种子文件.';
			dialog.confirm(header, message, '移除', 'transmission.removeTorrents', torrents);
		}
		else
		{
			var header = '你确定要移除选择的 ' + torrents.length + ' 个任务吗?',
			    message = '移除后, 若要继续下载任务需提供这些任务的种子文件';
			dialog.confirm(header, message, '移除', 'transmission.removeTorrents', torrents);
		}
	},
	...
	promptToRemoveTorrentsAndData:function(torrents)
	{
		...
		{
			var torrent = torrents[0],
			    header = '您确定要移除 ' + torrent.getName() + ' 和已下载的数据吗?',
			    message = '所有属于此任务的数据将会被删除.';
			dialog.confirm(header, message, '移除', 'transmission.removeTorrentsAndData', torrents);
		}
		else
		{
			var header = '你确定要移除选择的 ' + torrents.length + ' 个任务和已下载的数据吗?',
			    message = '所有属于这些任务的数据将会被删除.';
			dialog.confirm(header, message, '移除', 'transmission.removeTorrentsAndData', torrents);
		}
	},
	...
	***/
	...
	updateFilterSelect: function()
	{
		...
		if (!this.filterTracker)
			str = '<option value="all" selected="selected">所有来源</option>';
		else
			str = '<option value="all">所有来源</option>';
