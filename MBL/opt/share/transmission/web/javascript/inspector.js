    ...
    updateInfoPage = function () {
        ...
        if(torrents.length < 1)
            ...
            if( !haveUnverified && !leftUntilDone )
                ...
                str = fmt.size(haveVerified) + ' / ' + fmt.size(sizeWhenDone) + ' (' + str +'%), ' + fmt.size(haveUnverified) + ' 未效验';
        }
        ...
        if(torrents.length < 1)
            ...
            if(!size)
                ...
                str = fmt.size(size) + ' (' + pieces.toStringWithCommas() + ' 分块 @ ' + fmt.mem(pieceSize) + ')';
            else
                str = fmt.size(size) + ' (' + pieces.toStringWithCommas() + ' 分块)';
        }
        ...
        if(torrents.length < 1)
            ...
            str = baseline ? '私有种子, 已禁用 DHT 和 PEX ' : '公共种子';
            for(i=0; t=torrents[i]; ++i) {
        ...
        if(torrents.length < 1)
            ...
            if(mixed_creator && mixed_date)
                ...
                str = creator + ' 创建于 ' + (new Date(date*1000)).toDateString();
        }
    ...
    getAnnounceState = function(tracker) {
        ...
        switch (tracker.announceState) {
            ...
            case Torrent._TrackerWaiting:
                ...
                s = '下次通告时间：' + Transmission.fmt.timeInterval(timeUntilAnnounce) + ' 后';
                break;
            ...
            case Torrent._TrackerInactive:
                ...
                break;
            default:
                s = '未知的通告状态: ' + tracker.announceState;
        }
        return s;
    },
        ...
        for (i=0; tor=torrents[i]; ++i)
        {
            ...
            for (j=0; tracker=trackers[j]; ++j)
            {
                ...
                html.push('<li class="inspector_tracker_entry ', parity, '"><div class="tracker_host" title="', sanitizeText(tracker.announce), '">',
                      ...
                      '<tr><th>做种者:</th><td>', (tracker.seederCount > -1 ? tracker.seederCount : na), '</td></tr>',
                      '<tr><th>吸血者:</th><td>', (tracker.leecherCount > -1 ? tracker.leecherCount : na), '</td></tr>',
                      '<tr><th>下载者:</th><td>', (tracker.downloadCount > -1 ? tracker.downloadCount : na), '</td></tr>',
                      '</table></li>');
            }
