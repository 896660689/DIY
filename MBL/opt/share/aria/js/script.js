...
$(function() {
	...
	if (location.search)
	{
		...
		if (pos != -1)
		{
			var auth = url.substring(0, pos);
			url = url.substring(pos + 1);
			pos = auth.indexOf(':');
			if (pos != -1)
			{
				server_conf.pass = auth.substring(pos + 1);
				auth = auth.substring(0, pos);
			}
			server_conf.user = auth;
		}

		pos = url.indexOf(":");
		if (pos != -1)
		{
			server_conf.port = url.substring(pos + 1);
			url = url.substring(0, pos);
		}

		server_conf.host = url;

		console.log
		(
			"user: " + server_conf.user + '\n' +
			"pass: " + server_conf.pass + '\n' +
...
function custom_global_settings() {
	...
	var metalink_settings = gen("磁力链设置");
