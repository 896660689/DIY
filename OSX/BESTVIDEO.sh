...
MakeVideo()
{
	...
	if [ -f "$subtitle" ]; then 
		...
		ffmpeg -i "$1" -y $audioOptions $cropOptions1,subtitles="$subtitle"$charsetOption $videoOptions "$outDir/${1%.*}.mp4" </dev/null
		if [ $? == 1 ]; then
			ffmpeg -i "$1" -y $audioOptions $cropOptions2,subtitles="$subtitle"$charsetOption $videoOptions "$outDir/${1%.*}.mp4" </dev/null
		fi
	else
		ffmpeg -i "$1" -y $audioOptions $cropOptions1 $videoOptions "$outDir/${1%.*}.mp4" </dev/null
		if [ $? == 1 ]; then
			ffmpeg -i "$1" -y $audioOptions $cropOptions2 $videoOptions "$outDir/${1%.*}.mp4" </dev/null
		fi
	fi
}
